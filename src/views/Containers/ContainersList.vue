<template>
  <q-page class="q-pa-md">
    <div v-if="tauriAvailable === false" class="text-negative">
      DB disponible solo en modo Tauri (desktop)
    </div>
    <div v-else>
      <div class="row items-center justify-between q-mb-md">
        <div class="text-h6">Contenedores</div>
        <q-btn color="primary" label="Crear" to="/containers/create" />
      </div>

      <q-table row-key="id" :rows="rows" :columns="columns" :loading="store.loading" flat bordered>
        <template #body-cell-actions="props">
          <q-td :props="props">
            <q-btn
              flat
              dense
              color="primary"
              label="Editar"
              :to="`/containers/${props.row.id}/edit`"
            />
            <q-btn flat dense color="negative" label="Eliminar" @click="confirmRemove(props.row)" />
          </q-td>
        </template>
      </q-table>

      <div v-if="store.error" class="text-negative q-mt-md">
        {{ store.error }}
      </div>
    </div>
  </q-page>
</template>

<script setup lang="ts">
import { computed, onMounted, ref } from 'vue';
import { useQuasar } from 'quasar';
import type { QTableColumn } from 'quasar';
import type { Container } from 'src/interfaces/entities';
import { useContainersStore } from 'src/stores/containers';
import { isTauri } from 'src/utils/isTauri';

const $q = useQuasar();
const store = useContainersStore();
const tauriAvailable = ref<boolean | null>(null);

const columns: QTableColumn<Container>[] = [
  { name: 'name', label: 'Nombre', field: 'name', align: 'left', sortable: true },
  {
    name: 'length',
    label: 'Largo',
    field: 'length',
    align: 'right',
    sortable: true,
  },
  { name: 'width', label: 'Ancho', field: 'width', align: 'right', sortable: true },
  { name: 'height', label: 'Alto', field: 'height', align: 'right', sortable: true },
  {
    name: 'max_weight',
    label: 'Peso Max',
    field: 'max_weight',
    align: 'right',
  },
  { name: 'actions', label: 'Acciones', field: () => '', align: 'right' },
];

const rows = computed(() => store.items);

function notifyError(err: unknown) {
  const message = err instanceof Error ? err.message : 'Error inesperado';
  $q.notify({ type: 'negative', message });
}

function confirmRemove(row: Container) {
  $q.dialog({
    title: 'Confirmar',
    message: '¿Eliminar contenedor?',
    cancel: true,
    persistent: true,
  }).onOk(() => {
    void (async () => {
      try {
        await store.remove(row.id);
        $q.notify({ type: 'positive', message: 'Contenedor eliminado' });
      } catch (err) {
        notifyError(err);
      }
    })();
  });
}

onMounted(async () => {
  tauriAvailable.value = await isTauri();
  if (!tauriAvailable.value) {
    return;
  }
  try {
    await store.fetchAll();
  } catch (err) {
    notifyError(err);
  }
});
</script>
