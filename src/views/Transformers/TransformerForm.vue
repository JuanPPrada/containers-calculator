...
import type { TransformerPayload } from 'src/interfaces/entities';
...
const form = ref<TransformerPayload>({
  name: '',
  length: 0,
  width: 0,
  height: 0,
...
const title = computed(() => (isEdit.value ? 'Editar transformador' : 'Crear transformador'));
...
async function loadExisting() {
  ...
  if (!item) {
    notifyError(new Error('Transformador no encontrado'));
    return;
  }
  form.value = {
    name: item.name,
...
async function onSubmit() {
  ...
  try {
    ...
    await router.push('/transformers');
  } catch (err) {
    notifyError(err);
  }
}
